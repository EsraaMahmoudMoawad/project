            ...
            <tbody>
                <?php foreach ($messages as $message) { ?>
                    ...
                    <tr  <?php if($message[3] == "not_reading"){echo "class='text-danger'";}?>>
                        ...
                        <td scope="row" ><?= $message[2] ?></td>

                        <td>

                            <!-- Button trigger modal -->
                            <a href="function/handel_order.php?id_edit=<?= $message[0] ?>">
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal<?php echo $key_cat['id']; ?>">
                                    Open
                                </button>
                            </a>

                            <a href="Order_info.php?id_info=<?= $message[0] ?>">
                                <button type="button" class="btn btn-success " data-toggle="modal" data-target="#exampleModal<?php echo $key_cat['id']; ?>">
                                    Order Info
                                </button>
                            </a>

                            <a href="function/handel_order.php?id_delete=<?= $message[0] ?>">
